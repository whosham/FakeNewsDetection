...
func judgeEventForLocationsFromReceiver(stub shim.ChaincodeStubInterface, args []string, clientID string, getLocations locationReceiver) (string, error) {
	...
	previousTrustWorthiness := 0.0
	value, err = stub.GetState(assessment.ID)
	if err != nil {
		return "", fmt.Errorf("failed to get assessment: %s", assessment.ID)
	}

	if value != nil {
		oldAssessment := Assessment{}
		err = json.Unmarshal(value, &oldAssessment)
		if err != nil {
			return "", err
		}
		previousTrustWorthiness = oldAssessment.Trustworthiness
	}
	...
	event.Trustworthiness += assessment.Trustworthiness - previousTrustWorthiness
...
func calculateUsersCloseness(stub shim.ChaincodeStubInterface, event Event, clientID string,
	...
	return getClosenessForLocations(providedLocations, event.Location, 0), nil
...
func getClosenessForLocations(locations []Location, c Coordinates, startTime int) float64 {

	if !ignoreFlawsInLocTrace {
		if containsFlaws(locations, startTime) {
			return minLocationFactor
		}
	}

	lastL := locations[len(locations)-1]
	d := distanceInMeters(lastL.Coordinates, c)
	dInKm := 0.001 * d
	closeness := math.Max(minLocationFactor, math.Pow(0.1, 0.01*dInKm))
...
func containsFlaws(locations []Location, startTime int) bool {
	...
	for _, l := range locations {
		locTime, err := strconv.Atoi(l.Timestamp)
		if err != nil {
			return true
		}
		if locTime > time+60*60 {
			time = time + 60*60
			flawCounter++
			continue
		}
		dist := distanceInMeters(lastLoc.Coordinates, l.Coordinates)
		speed := dist / float64(locTime+time)
		if speed > maxAllowedSpeedInMeterPerSecond {
			return true
		}
		time = locTime
	}

	return flawCounter > 3
}
