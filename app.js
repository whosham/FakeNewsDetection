...
app.post('/channels/:channelName/chaincodes/:chaincodeName', async function(req, res) {
	...
	logger.debug('args  : ' + args); 
        logger.debug('Hey!') ;
        logger.debug(typeof peers) ;
        logger.debug( peers) ;
        logger.debug( "Before" +  typeof args ) ;
        args = [args]; 
        logger.debug( "After" + typeof args ) ; 
        logger.debug('args  : ' + args); 
       	if (args instanceof String) logger.debug('String!');
	if (args instanceof Object) logger.debug('Object!');
	if (args instanceof Array) logger.debug('Array!');
	if (!chaincodeName) {
		res.json(getErrorMessage('\'chaincodeName\''));
		return;
	}
...
//app.post('/channels/:channelName/chaincodes/:chaincodeName', async function(req, res) {
	...
	let peer = req.query.peer;
        logger.debug('args : ' + args);
        logger.debug("before json" +typeof args)
      //  args=JSON.stringify(args);
        args.replace(/^\s+|\s+$/g, '');
        args = [args];
	...
	logger.debug('fcn : ' + fcn);
	logger.debug('args : ' + args);
        logger.debug("remove spaces" +typeof args)
